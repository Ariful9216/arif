...
class MinuteOfferController extends GetxController {
  ...
  Future<void> getSliders() async {
    try {
      ...
      // Fetch both minute and all type sliders
      final minuteResponse = await Repository.getSliders(type: 'minute');
      final allResponse = await Repository.getSliders(type: 'all');
      
      List<SliderItem> combinedSliders = [];
      
      if (minuteResponse != null && minuteResponse.success) {
        combinedSliders.addAll(minuteResponse.data.where((slider) => slider.isActive));
      }
      ...
      print("Minute sliders loaded: ${sliderList.length} items (minute + all types)");
    } catch (e) {
      print("Error loading minute sliders: $e");
    } finally {
  ...
  getOffers() async {
    ...
    try{
      ...
      if(tempData.containsKey(selectedOperators.value)){
        ...
        final response = await Repository.getOffers(operator: selectedOperators.value, type: "minute");
        if (response != null) {
  ...
  getPendingOrder() async {
    ...
    allPendingOrderList=VarConstants.pendingOrderModel!.data?.where((element) => element.offer.offerType=="minute",).toList() ?? [];
    pendingOrderList=allPendingOrderList;
    isPendingLoading.value = false;
  }
  ...
  getHistoryOrder() async {
    ...
    allHistoryOrderList=VarConstants.historyOrderModel!.data?.where((element) => element.offer.offerType=="minute",).toList() ?? [];
    historyOrderList=allHistoryOrderList;
    isHistoryLoading.value = false;
  }
