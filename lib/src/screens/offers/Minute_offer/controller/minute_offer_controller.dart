...
class MinuteOfferController extends GetxController {
  ...
  Future<void> getSliders() async {
    try {
      ...
      // Fetch both minute and all type sliders in PARALLEL
      final results = await Future.wait([
        Repository.getSliders(type: 'minute'),
        Repository.getSliders(type: 'all'),
      ]);

      List<SliderItem> combinedSliders = [];

      // Process minute sliders (first result)
      final minuteResponse = results[0];
      if (minuteResponse != null && minuteResponse.success) {
        combinedSliders.addAll(
          minuteResponse.data.where((slider) => slider.isActive),
        );
      }
      ...
      print(
        "Minute sliders loaded: ${sliderList.length} items (minute + all types)",
      );
    } catch (e) {
  ...
  getPendingOrder() async {
    ...
    allPendingOrderList =
        VarConstants.pendingOrderModel!.data
            ?.where((element) => element.offer.offerType == "minute")
            .toList() ??
        [];
    pendingOrderList = allPendingOrderList;
    isPendingLoading.value = false;
