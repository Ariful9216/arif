...
class ComboOfferController extends GetxController {
  ...
  Future<void> getSliders() async {
    try {
      ...
      // Fetch both combo and all type sliders in PARALLEL
      final results = await Future.wait([
        Repository.getSliders(type: 'combo'),
        Repository.getSliders(type: 'all'),
      ]);

      List<SliderItem> combinedSliders = [];

      // Process combo sliders (first result)
      final comboResponse = results[0];
      if (comboResponse != null && comboResponse.success) {
        combinedSliders.addAll(
          comboResponse.data.where((slider) => slider.isActive),
        );
      }
      ...
      print(
        "Combo sliders loaded: ${sliderList.length} items (combo + all types)",
      );
    } catch (e) {
  ...
  getPendingOrder() async {
    ...
    allPendingOrderList =
        VarConstants.pendingOrderModel!.data
            ?.where((element) => element.offer.offerType == "combo")
            .toList() ??
        [];
    pendingOrderList = allPendingOrderList;
    isPendingLoading.value = false;
